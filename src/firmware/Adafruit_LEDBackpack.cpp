...
void Adafruit_7segment::turnOffDigit(uint8_t d){
  if (d > 4) return;

  //writeDigitRaw((uint8_t)d, (uint8_t)0);// int(0b0000000000000000));
  //writeDigitRaw(d, numbertable[0] | (true << 7));
  displaybuffer[d] = 0b0000000000000000;
}

void Adafruit_7segment::writeDigitNumMirrored(uint8_t d, uint8_t num, boolean dot) {
  if (d > 4) return;
